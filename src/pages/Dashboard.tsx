  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-6">
      ...
      <header className="flex items-center justify-between">
        ...
        <button
          onClick={logout}
          className="rounded-xl hidden md:block border px-3 py-2 hover:bg-gray-100"
        >
          Sign Out
        </button>
        <a onClick={logout} className="text-[#367AFF] underline md:hidden">
          Sign Out
        </a>
      </header>
      ...
      <div className="flex flex-col items-center gap-3">
        <img
          ...
          alt="Profile"
          className="w-30 h-30 rounded-full hidden md:block border"
        />
        <div className="flex flex-col w-full max-w-md mx-auto border border-[#D9D9D9] px-3 py-5 shadow-md rounded-lg">
          <p className="text-xl font-semibold mb-2 text-[#232323]">
            Welcome, {user?.name || 'Guest User'} !
          </p>
          <h2 className="text-sm text-[#232323]">
      ...
      <div className="flex justify-center">
        <button
          ...
          className="rounded-xl bg-blue-500 w-full text-white px-4 py-2 hover:bg-blue-600 transition"
        >
          Create Note
        </button>
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-md relative">
            ...
            <textarea
              ...
              className="w-full border rounded-xl p-2 h-28 mb-3"
            />
            {error && <p className="text-red-600 text-sm mb-2">{error}</p>}
            <button
              onClick={createNote}
              className="w-full rounded-xl bg-black text-white px-4 py-2 hover:bg-gray-800"
            >
              Add Note
            </button>
          </div>
